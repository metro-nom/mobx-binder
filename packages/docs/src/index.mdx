...
A [Binder](packages/mobx-binder-core/src/model/binder/Binder.ts) manages form state for a set of fields represented by "dumb" observable field instances implementing the [FieldStore](packages/mobx-binder-core/src/model/fields/FieldStore.ts) interface. It is configured via a fluent api like this:
...
A simple [example](packages/sample/src/app/forms/FormField.tsx) of how to render such fields can be found in the [sample code](packages/sample).
